...
async function ensureSchema(db) {
  ...
  `);
    ...
    `);
        ...
        console.warn("[skillsearch] pgvector not available; embedding column/index skipped. " +
...
async function searchSkills(db, query, limit, threshold) {
  ...
  `);
    ...
    if (hasEmbedding.rowCount && hasEmbedding.rowCount > 0 && RERANK_BASE_URL && RERANK_API_KEY) {
       ...
       LIMIT $3`, [vecStr, threshold, limit * 3] // fetch extra for rerank
        );
...
function createServer() {
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
        const tools = [
            {
                name: "skill_search",
                description: "Search for Agent Skills by semantic query. Returns matching skills with their frontmatter metadata, SKILL.md path, and paths to reference/script files. Use this to discover available skills before reading their full content.",
                inputSchema: {
                    type: "object",
                    properties: {
                        query: {
            ...
            {
                name: "skill_list",
                description: "List all indexed skills with their names, descriptions, and directory paths.",
                inputSchema: {
                    type: "object",
                    properties: {},
                    required: [],
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        switch (request.params.name) {
            ...
            case "skill_list": {
                const result = await db.query(`SELECT skill_name, skill_dir, skill_md_path, frontmatter
           FROM skills
           ORDER BY skill_name`);
