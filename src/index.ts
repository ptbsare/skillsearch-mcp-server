...
const RERANK_BASE_URL =
  process.env.API_BASE_URL ?? process.env.SKILLSEARCH_RERANK_BASE_URL ?? "";
const RERANK_API_KEY =
  process.env.API_KEY ?? process.env.SKILLSEARCH_RERANK_API_KEY ?? "";
const RERANK_MODEL =
...
interface SkillEntry {
  ...
  allFiles: string[];       // ALL files under skill dir (recursive, excluding SKILL.md itself)
  skillMdPathAbsolute: string;
}
...
function getPool(): PoolType {
  if (!pool) {
    if (!DB_URL) {
      throw new Error(
        "DB_URL (or SKILLSEARCH_DB_URL) environment variable is not set"
      );
    }
...
async function ensureSchema(db: PoolType): Promise<void> {
  ...
  try {
    ...
    if (col.rowCount === 0) {
      ...
      try {
        ...
        if (existing.rowCount && existing.rows[0].atttypmod !== embDims) {
          console.error(
            `[skillsearch] Resizing embedding column from ${existing.rows[0].atttypmod} to ${embDims}`
          );
    ...
    console.warn(
      `[skillsearch] pgvector not available or error: ${err}. ` +
...
function discoverSkills(rootDir: string): SkillEntry[] {
  ...
  for (const entry of entries) {
    ...
    const allFiles = collectAllFilesInDir(skillDir).filter(
      (f) => path.resolve(f) !== path.resolve(skillMdPath)
    );
    ...
    console.error(
      `[skillsearch] Discovered skill "${name}": ${allFiles.length} files in ${skillDir}`
    );
  }
...
function collectAllFilesInDir(dir: string): string[] {
...
async function searchSkills(
  ...
  if (
    ...
    console.error(
      `[skillsearch] Vector search returned ${rows.length} results (threshold=${threshold})`
    );
  } else {
    ...
    const tools: Tool[] = [
      {
        name: "skill_search",
        description:
          "Search for Agent Skills by semantic query. Returns matching skills with frontmatter metadata, SKILL.md absolute path, and ALL file paths under the skill directory (references, scripts, assets, etc.). Use this to discover available skills before reading their full content.",
        inputSchema: {
          type: "object",
          properties: {
            query: {
      ...
      },
      {
        name: "skill_list",
        description:
          "List all indexed skills with their names, descriptions, directory paths, and file counts.",
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (request.params.name) {
                ...
                text: JSON.stringify(
                  {
                    ...
                    totalResults: 0,
                    message:
                      "No matching skills found. Try a different query or lower the threshold.",
                    skills: [],
                  },
                  null,
                  2
      ...
      case "skill_list": {
        ...
        return {
          content: [
            { type: "text" as const, text: JSON.stringify({ total: skills.length, skills }, null, 2) },
          ],
        };
      }
      ...
      case "skill_reindex": {
        if (!SKILLS_DIR) {
          return {
            content: [
              { type: "text" as const, text: "Error: SKILLSEARCH_SKILLS_DIR is not set" },
            ],
            isError: true,
          };
        }
...
async function main(): Promise<void> {
  ...
  if (skills.length > 0) {
    const indexed = await indexSkills(db, skills);
    console.error(
      `[skillsearch] Indexed ${indexed} skills (${skills.length - indexed} unchanged)`
    );
  }
